...
CreateDirectory(char *name)
{
	...
	DEBUG(dbgFile, "Creating Empty Directory\n");
	
	if(!kernel->fileSystem->CreateDirectory(name)) {
		printf("couldn't create new directory %s.\n", name);
		return;
	}
...
main(int argc, char **argv)
{
    ...
    if (removeFileName != NULL) {
		if(!recursiveRemoveFlag)
			...
			kernel->fileSystem->RecurRemoveDirectory(removeFileName);
    }
    ...
    if (dirListFlag) {
		if(!recursiveListFlag)
			kernel->fileSystem->ListDirectory(listDirectoryName);
		else
			kernel->fileSystem->RecurListDirectory(listDirectoryName);
    }
	if (mkdirFlag) {
		// MP4 mod tag
